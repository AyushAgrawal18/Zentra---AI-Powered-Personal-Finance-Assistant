...
beforeEach(() => {
  ...
  (repo.listGoals as jest.Mock) = jest
    .fn()
    .mockResolvedValue({ rows: [mockGoal], total: 1 });
...
describe("POST /api/v1/goals", () => {
  ...
  it("creates a goal successfully", async () => {
    ...
    expect(res.body.data.id).toBe("55555555-5555-4555-8555-555555555555");
  ...
  it("returns 400 when targetDate is not a real calendar date", async () => {
    const res = await request(app)
      .post("/api/v1/goals")
      .set(AUTH_HEADER)
      .send({ ...validBody, targetDate: "2027-02-30" });
    expect(res.status).toBe(400);
  });
...
describe("GET /api/v1/goals", () => {
  it("returns paginated list of goals", async () => {
    const res = await request(app).get("/api/v1/goals").set(AUTH_HEADER);
...
describe("GET /api/v1/goals/:id", () => {
  it("returns goal details", async () => {
    const res = await request(app)
      .get("/api/v1/goals/55555555-5555-4555-8555-555555555555")
      .set(AUTH_HEADER);
...
describe("PATCH /api/v1/goals/:id", () => {
  it("updates goal successfully", async () => {
    const res = await request(app)
      .patch("/api/v1/goals/55555555-5555-4555-8555-555555555555")
      .set(AUTH_HEADER)
...
describe("DELETE /api/v1/goals/:id", () => {
  it("soft deletes goal successfully", async () => {
    const res = await request(app)
      .delete("/api/v1/goals/55555555-5555-4555-8555-555555555555")
      .set(AUTH_HEADER);
...
describe("POST /api/v1/goals/:id/contribute", () => {
  it("adds contribution successfully", async () => {
    const res = await request(app)
      .post("/api/v1/goals/55555555-5555-4555-8555-555555555555/contribute")
      .set(AUTH_HEADER)
...
describe("GET /api/v1/goals/:id/progress", () => {
  it("returns goal progress", async () => {
    const res = await request(app)
      .get("/api/v1/goals/55555555-5555-4555-8555-555555555555/progress")
      .set(AUTH_HEADER);

    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data.goalId).toBe("55555555-5555-4555-8555-555555555555");
    expect(res.body.data.targetAmount).toBe(120000);
    expect(res.body.data.currentAmount).toBe(35000);
    expect(res.body.data.remainingAmount).toBe(85000);
  });
